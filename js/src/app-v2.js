...
function isInstantiated(element, actions) {
	...
	var Manager = $.data(element, pluginName);
...
Constructors.ElementView = Backbone.View.extend({
	...
	initialize: function(opts) {
		...
		// Save reference to elModel, elCollection, Manager
	...
	// This method appends the view's $el (an 'li') to the $ul of the Manager.
	// Also, it applies the 'elCss' to the $el.
	// The 'elCss' property may be set both in the Manager level [Manager.set('elCss', elCss),
	// becoming a default elCss for all views, 
	// and in the element level [Manager.addElement({id: id ,elCss: elCss}, 
	// being used only at the instantiated element.
	_basicRender: function() {
		var $ul = this.Manager.els.$ul,
			elCss = _.extend({}, defaultCss, this._getOpt('elCss') );
			
		this.$el.attr('id', this._getOpt('id') );
		$ul.append(this.$el);
	...
	_setDisplay: function(model, value) {
		var _this = this,
			model = this.elModel,
			displayValue = model._getOpt('display');
		...
		switch (displayValue) {
			case 'show':
				...
				this.elModel.once('show-start', function() {
					$.when( _this._show() ).then(function() {
				...
				waitFor.call(this, {
					events: [
						{
							emitter: this.Manager.events, 
							name: 'show-prep-ready',
							check: function() {
								return _this.Manager.get('transition-status') == 'show-prep-ready';
							}
						},
						{
			...
			case 'hide':
				var overlap = this.Manager.get('overlap');
				
				if (overlap > 0 && overlap <= 1) {
					var duration = this._getOpt('duration'),
	...
	// The ._show() method is called by ._setDisplay method.
	...
	_show: function() {
		var _this = this,
			defer = $.Deferred(),
			asyncDefer,
			type = this._getOpt('transitionType'),
			duration = this._getOpt('duration');
	...
	// The ._hide() method is called by ._setDisplay method.
	...
	// The ._getOpt() method is an alias to the elModel's _getOpt method.
	_getOpt: function(opt) {
		return this.elModel._getOpt(opt);
	},
	
	// This is an no-op. The .setup() method may be overwritten
	// if you wish to do anything immediately before the ._show()
	// method is called.
	setup: function() {},
	beforeShow: function() {},
	afterShow: function() {},
	...
	// if you wish to do anything AFTER the ._hide() method is complete.
	teardown: function() {},
	beforeHide: function() {},
	afterHide: function() {}
});
...
Constructors.ElementModel = Backbone.Model.extend({
	...
	initialize: function(attributes, opts) {
		...
		this.Manager = this.elCollection.Manager;
		
		// proxy all this model's events to the Manager
		this.on('all', function(event) {
			_this.Manager.events.trigger(event, _this);
		});
		
		// all changes on the 'remote-status' should be 
		// fired by the _eventedSetting
		this.on('change:render-status', this._eventedSetting);
		
		// all changes on the 'transition-status' should be 
		// fired by '_eventedSetting'
		this.on('change:transition-status', this._eventedSetting);
		
		// set view listeners everytime it is changed
		this.on('change:view', this._setViewListeners);
		
		// set the view to be instantiated only when the 
		// 'remote-ready' event is fired
		this.on('remote-ready', this._setView);
		
		// load all dependencies if this is a priority view (if any)
		this._loadDependencies();
	},
	
	// ._eventedSetting handles 'change:key' events by triggering a event the same
	...
	_eventedSetting: function(model, value, options) {
		this.trigger(value, this);
	},
	
	// ._loadDependencies() method builds an $.Deferred object that is returned
	...
	_loadDependencies: function() {
		var id = this.get('templateId') || this.get('id'),
			_this = this,
			dependencies = this._getOpt('dependencies'),
			deferrals = [];
	...
	_setViewListeners: function() {
		...
		// The 'view-instantiated' event is fired by the '._setView()' method
		...
		// the view should update it_this by calling _fillIn.
		...
		this.once('render-ready', function() {
			...
			view.listenTo(_this, 'change:fillInData', view._fillIn);
	...
	// ._getOpt() method tries to get the attribute in this model and
	// if it is not successful, it gets the same option on the Manager 
	// where the default and global settings are stored.
	_getOpt: function(opt) {
		return this.get(opt) || this.Manager.get(opt);
	}
});
...
// Plugin is the constructor of the object to be stored in $.data(element, 'Manager')
...
function Plugin(element, options) {
	...
	this.elCollection = new Constructors.ElementCollection([], { Manager: this });
...
Plugin.prototype.addElement = function(info) {
	var elInfo = _.extend({}, { Manager: this, elCollection: this.elCollection }, info),
		route = info.route;
...
$.fn[pluginName] = function ( method_or_options ) {
	...
	if ( typeof method_or_options === 'object' || !method_or_options ) {
		...
		this.each(function() {
			...
			isInstantiated(this, {
				yes: function(Manager) {
					// run the method requested using the Manager context
					responseArr.push( Manager[method_or_options].apply(Manager, args) );
				},
				no: function() {
					// if not, throw error
